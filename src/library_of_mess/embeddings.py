...
def save_embeddings(store_path: Path, stems: list[str], vectors: np.ndarray) -> None:
    ...
    np.savez(tmp, stems=np.array(stems), vectors=vectors.astype(np.float32))
    tmp.replace(store_path)


def update_embeddings(
    thumb_paths: list[Path], encode_images: ImageEncoder, store_path: Path
    ...
    cached, so repeated builds only cost the newly added videos. When the
    encoder's output dimension changed (model swap), the whole store is
    rebuilt with the new encoder — old and new vectors must never mix.
    ...
    save_embeddings(store_path, stems, matrix)
    return stems, matrix
